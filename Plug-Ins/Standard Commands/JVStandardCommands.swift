...
public class StandardCommands : NSObject, MVChatPlugin {
	...
	public func handleFileSendWithArguments(_ arguments: String!, forConnection connection: MVChatConnection!) -> Bool {
		...
		if let path = path where path.length > 0 && !FileManager.default().fileExists(atPath: path as String, isDirectory: &directory) {
			return true
		}
		if directory {
			return true
	...
	public func handleServerConnectWithArguments(_ arguments: String!) -> Bool {
		if let _ = arguments?.range(of: "://"), url = URL(string: arguments) {
	...
	public func handlePartWithArguments(_ arguments: String!, forConnection connection: MVChatConnection!) -> Bool {
		...
		let reason = AttributedString(string: message)
	...
	public func handleMessageCommand(_ command: String!, withMessage message: AttributedString!, forConnection connection: MVChatConnection!, alwaysShow always: Bool) -> Bool {
		var to1: NSString?
	...
	public func handleMassMessageCommand(_ command: String, withMessage message: AttributedString!, forConnection connection: MVChatConnection!) -> Bool {
	...
	public func handleMassAwayWithMessage(_ message: AttributedString?) -> Bool {
		for item in MVConnectionsController.default.connectedConnections {
	...
	public func handleIgnoreWithArguments(_ args1: String, inView view: JVChatViewController!) -> Bool {
		...
		if message {
			if let range1 = args.range(of: "\""), range2 = args.range(of: "\"", options: .backwardsSearch) {
				messageString = args.substring(with: range1.upperBound ..< range2.lowerBound)
			} else if let range1 = args.range(of: "/"), range2 = args.range(of: "/", options: .backwardsSearch) {
				messageString = args.substring(with: range1.upperBound ..< range2.lowerBound)
			} else {
	...
	public func handleUnignoreWithArguments(_ args1: String, inView view: JVChatViewController!) -> Bool {
		...
		if message {
			if let range1 = args.range(of: "\"") {
				let range2 = args.range(of: "\"", options: .backwardsSearch)!
				messageString = args.substring(with: range1.upperBound ..< range2.lowerBound)
	...
	public override func processUserCommand(_ command1: String, withArguments arguments: AttributedString, to connection: MVChatConnection?, inView view: JVChatViewController?) -> Bool {
		...
		if isChatRoom {
			...
			case "kick":
				...
				var reason: AttributedString? = nil;
				...
				if let user = room?.target?.memberUsers(withNickname: member1 as String).first {
			...
			case "kickban", "bankick", "bk", "kb":
				...
				if member.hasCaseInsensitiveSubstring("!") || member.hasCaseInsensitiveSubstring("@")  {
					...
					user = room?.target?.memberUsers(withNickname: member).first
				}
				
				if let user = user {
					room?.target?.addBan(for: user)
					room?.target?.kickOutMemberUser(user, forReason: reason)
					return true
				}
				return false;
			...
			case "op":
				...
				for arg in args {
					if arg.characters.count > 0 {
						if let user = room?.target?.memberUsers(withNickname: arg).first {
			...
			case "dequiet":
				...
				for arg in args {
					if arg.characters.count > 0 {
						if let user = room?.target?.memberUsers(withNickname: arg).first {
							room?.target?.remove(.disciplineQuietedMode, forMemberUser: user)
						}
					}
				}
				return true
			...
			case "ban":
				...
				for arg1 in args {
					...
					if arg.hasCaseInsensitiveSubstring("!") || arg.hasCaseInsensitiveSubstring("@") || room?.target?.memberUsers(withNickname: arg) == nil {
						if !arg.hasCaseInsensitiveSubstring("!") && arg.hasCaseInsensitiveSubstring("@") {
						arg = "*!*" + arg;
						}
						user = MVChatUser.wildcardUser(from: arg)
					} else {
						user = room?.target?.memberUsers(withNickname: arg).first
					}
			...
			case "unban":
				...
				for arg1 in args {
					...
					if let user = user {
						room?.target?.removeBan(for: user)
					}
				}
				return true
